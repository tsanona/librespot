use crate::{
    LoadContextOptions, LoadRequestOptions, PlayContext, SpircCommand, SpircPlayerArgs, context_resolver::{ContextAction, ContextResolver, ResolveContext}, core::{
    ...
    }, model::{LoadRequest, PlayingTrack, SpircPlayStatus}, playback::{
        mixer::Mixer,
    ...
    }, spirc::task::{CONTEXT_FETCH_THRESHOLD, SPIRC_COUNTER, SpircTask, UPDATE_STATE_DELAY, VOLUME_UPDATE_DELAY}, state::{
        ConnectState, context::{ContextType, ResetContext}, provider::IsProvider
    }
};

use crate::SpircError;
...
impl SpircTask for PlayerTask {
    type Args = SpircPlayerArgs;
    type Interface = UnboundedSender<SpircCommand>;

    async fn new(
        args: Self::Args
    ) -> Result<(Self, UnboundedSender<SpircCommand>), Error> {
        let spirc_id = SPIRC_COUNTER.fetch_add(1, Ordering::AcqRel);
        debug!("new Spirc[{spirc_id}]");

        let SpircPlayerArgs { config, session, credentials, player, mixer } = args;
